# ============================================================
# Minimum Detectable Effect (MDE) for per-capita antibiotic use
# One-sample Poisson rate test, simple random sample
# ============================================================
#
# ASSUMPTIONS
#   - Antibiotic use follows a Poisson process (courses/person/year)
#   - Two visit streams: public health centres (PHC) and other vendors
...
scenarios$lambda0 <- with(scenarios,
  visits * phc_share * phc_rx_prob +
  visits * vendor_share * vendor_rx_prob
)

# --- Solve for MDE numerically (exact Poisson power equation) ---
# Power condition: sqrt(n) * (lambda0 - lambda1) = z_a*sqrt(lambda0) + z_b*sqrt(lambda1)
# Rearranged as root of: f(lambda1) = 0
...
# --- Output ---
cat("\n=== MDE for per-capita antibiotic use (alpha=0.05, power=80%, n=500) ===\n\n")
